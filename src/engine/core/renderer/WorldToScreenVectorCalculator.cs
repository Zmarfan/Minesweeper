...
public static class WorldToScreenVectorCalculator {
    public static unsafe SDL.SDL_Rect CalculateTextureDrawPosition(WorldToScreenVectorParameters p) {
        ...
        SDL.SDL_Rect rect = new() {
            ...
            w = (int)(p.surface->w * p.scale * (1 /p.settings.camera.Size)),
            h = (int)(p.surface->h * p.scale * (1 /p.settings.camera.Size))
        };
        return rect;
    }
